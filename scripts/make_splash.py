...
ICON_SVG = SCRIPT_DIR.parent / 'assets' / 'icon.svg'
...
# Use pre-built assets/icon.png if available (required on Windows where
# svg2png.sh cannot run). Regenerate it on macOS/Linux with:
#   scripts/svg2png.sh assets/icon.svg assets/icon.png 256 256
prebuilt_png = SCRIPT_DIR.parent / 'assets' / 'icon.png'
